...
import {
  ...
  Sun, Moon // Aggiunte icone per il toggle
...
<template>
  <div class="min-h-screen bg-background text-foreground font-sans antialiased selection:bg-primary/20 transition-colors duration-300">
    ...
    <nav class="sticky top-0 z-50 border-b border-border bg-background/80 backdrop-blur-lg">
      <div class="container mx-auto px-6 md:px-8 py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <div class="flex items-center gap-4 sm:gap-6 flex-shrink-0">
          ...
          <NuxtLink to="/login" class="text-sm md:text-base font-medium text-muted-foreground hover:text-foreground transition">Client Log in</NuxtLink>
          <Button variant="outline" @click="scrollToWaitlist" class="border-border rounded-full hover:bg-foreground hover:text-background transition-all px-6 sm:px-8 py-2 font-bold">
            Prenota Demo
          </Button>
        </div>
      </div>
    ...
    <main>
      <section class="relative pt-32 pb-40 px-6 overflow-hidden">
        <div class="container mx-auto text-center relative z-10">
          <div class="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-accent border border-border text-muted-foreground text-xs font-medium mb-10">
          ...
          <h1 class="text-4xl md:text-6xl lg:text-7xl font-black tracking-tighter leading-tight mb-8 md:mb-12 uppercase">
            DATI D'ÉLITE PER<br />
            <span class="text-muted-foreground/30 italic font-light">ATLETI D'ÉLITE.</span>
          </h1>
          ...
          <div class="flex flex-col sm:flex-row justify-center gap-4 w-full max-w-xl mx-auto px-4">
            <Button size="lg" @click="scrollToWaitlist" class="w-full sm:w-auto h-14 sm:h-16 px-6 sm:px-12 rounded-2xl bg-foreground text-background hover:opacity-90 text-lg md:text-xl font-bold group transition-transform active:scale-95">
              <span>Inizia come Coach</span>
              <ArrowRight class="ml-2 size-5 md:size-6 group-hover:translate-x-1 transition inline-block" />
            </Button>
            <Button size="lg" @click="scrollToWaitlist" variant="ghost" class="w-full sm:w-auto h-14 sm:h-16 px-6 sm:px-10 text-lg text-muted-foreground hover:text-foreground font-semibold">
              Esplora le Funzioni
            </Button>
          </div>
        </div>
        <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[500px] h-[500px] bg-primary/10 blur-[120px] rounded-full -z-10"></div>
      </section>

      <section class="py-32 border-y border-border bg-accent/30">
        <div class="container mx-auto px-6">
          <div class="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
            <div v-for="f in coreFeatures" :key="f.title" class="group p-10 rounded-[2.5rem] border border-border bg-card hover:bg-accent transition-all duration-500 shadow-sm">
              <div class="size-14 rounded-2xl bg-accent flex items-center justify-center mb-8 group-hover:bg-foreground group-hover:text-background transition-colors duration-500">
                <component :is="f.icon" class="size-7" />
              </div>
              <h3 class="text-2xl font-bold mb-4 tracking-tight uppercase">{{ f.title }}</h3>
              <p class="text-muted-foreground leading-relaxed text-sm">{{ f.desc }}</p>
            </div>
          </div>
        </div>
      </section>
          ...
          <div class="grid lg:grid-cols-2 gap-24 items-center">
            <div class="space-y-10">
              ...
              <div class="grid grid-cols-2 gap-10 pt-6">
                <div class="space-y-2">
                  <div class="text-sm font-black uppercase tracking-widest text-foreground/80">Data Integrity</div>
                  <div class="text-xs text-muted-foreground font-mono italic">Backup orari automatizzati e sincronizzazione cloud.</div>
                </div>
                <div class="space-y-2">
                  <div class="text-sm font-black uppercase tracking-widest text-foreground/80">Security</div>
                  <div class="text-xs text-muted-foreground font-mono italic">Crittografia 256-bit per la massima privacy.</div>
                </div>
              </div>
            </div>
            <div class="relative group">
              <div class="absolute -inset-1 bg-gradient-to-r from-primary to-purple-600 rounded-[3rem] blur opacity-10 group-hover:opacity-20 transition"></div>
              <div class="relative aspect-video bg-accent border border-border rounded-[2.5rem] flex items-center justify-center italic text-muted-foreground/40 text-sm overflow-hidden">
                <div class="absolute inset-0 bg-[url('/dashboard_preview.png')] opacity-10 dark:opacity-5 pointer-events-none"></div>
                <img src="/favicon.ico" class="size-10 absolute opacity-10 grayscale" />
                <span class="z-10 font-bold uppercase tracking-widest">Dashboard Preview Mode</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <section class="py-40 border-t border-border bg-accent/10">
        <div class="container mx-auto px-6 text-center">
          ...
          <div class="grid md:grid-cols-3 gap-20 max-w-5xl mx-auto">
            <div v-for="(step, i) in [
              { icon: Watch, title: 'Wearable Sync', date: 'Q3 2026' },
              { icon: BrainCircuit, title: 'AI Insights', date: 'Q4 2026' },
              { icon: Smartphone, title: 'Mobile App', date: 'Q1 2027' }
            ]" :key="i" class="space-y-6">
              ...
              <p class="text-xs font-mono text-muted-foreground italic">{{ step.date }}</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section id="waitlist" class="py-48 px-6 bg-background border-t border-border">
        <div class="max-w-md mx-auto">
          <div v-if="isSubmitted" class="text-center space-y-6 py-10 animate-in fade-in zoom-in duration-500">
            <div class="size-20 bg-foreground text-background rounded-3xl flex items-center justify-center mx-auto shadow-xl">
              <CheckCircle2 class="size-10" />
            </div>
            <div class="space-y-2">
              <h2 class="text-3xl font-black uppercase italic tracking-tight">Richiesta Ricevuta</h2>
              <p class="text-muted-foreground text-sm leading-relaxed">
                Grazie Coach. Ti contatteremo entro 48 ore per l'accesso prioritario.
              </p>
            </div>
            <Button @click="isSubmitted = false; email = ''" variant="ghost" class="text-muted-foreground hover:text-foreground text-[10px] uppercase tracking-widest">
              Invia un'altra mail
            </Button>
          </div>
          ...
          <div v-else>
            <div class="text-center mb-12">
              ...
              <p class="text-muted-foreground text-sm italic">Posti limitati per i primi 20 coach partner.</p>
            </div>

            <form @submit.prevent="handleFormSubmit" class="space-y-6">
              <div class="space-y-2">
                <Label class="text-[10px] font-black uppercase tracking-widest text-muted-foreground ml-1">Email Professionale</Label>
                <Input
                  v-model="email"
                  type="email"
                  required
    ...
    <footer class="py-20 border-t border-border bg-accent/20">
      <div class="container mx-auto px-6 flex flex-col md:flex-row justify-between items-center gap-6 text-[10px] font-mono uppercase tracking-[0.2em] opacity-60">
        <div class="flex items-center gap-3">
          <img src="/favicon.ico" class="size-4 dark:invert-0 invert" /> Athlete Hub
        </div>
        <div class="flex gap-10">
          <NuxtLink to="/datapolicy" class="hover:text-foreground transition">Privacy & Data Policy</NuxtLink>
          <a href="https://github.com/manuelenzo" target="_blank" class="hover:text-foreground transition font-bold underline">Github</a>
        </div>
