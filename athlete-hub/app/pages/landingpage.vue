...
const showcaseTabs = [
  { label: 'Dashboard', descKey: 'landingpage.showcase.tab0Desc', img: '/dashboard_preview.png' },
  { label: 'Agenda', descKey: 'landingpage.showcase.tab1Desc', img: '/calendar.png' },
  { label: 'Analytics', descKey: 'landingpage.showcase.tab2Desc', img: '/dettagli.png' },
  { label: 'Sleep Tracking', descKey: 'landingpage.showcase.tab3Desc', img: '/sleep.png' },
  { label: 'AI Assistant', descKey: 'landingpage.showcase.tab4Desc', img: '/ai-chat.png' },
]

// Pain points
const painItems = [
  { icon: FileSpreadsheet, titleKey: 'landingpage.pain.item1Title', descKey: 'landingpage.pain.item1Desc', color: 'bg-red-500/10 text-red-500' },
  { icon: AlertTriangle, titleKey: 'landingpage.pain.item2Title', descKey: 'landingpage.pain.item2Desc', color: 'bg-orange-500/10 text-orange-500' },
  { icon: Mail, titleKey: 'landingpage.pain.item3Title', descKey: 'landingpage.pain.item3Desc', color: 'bg-yellow-500/10 text-yellow-500' },
  { icon: TrendingDown, titleKey: 'landingpage.pain.item4Title', descKey: 'landingpage.pain.item4Desc', color: 'bg-red-500/10 text-red-400' },
]

// Benefits
const benefits = [
  { icon: Clock, titleKey: 'landingpage.benefits.b1Title', descKey: 'landingpage.benefits.b1Desc', color: 'bg-emerald-500/10 text-emerald-500' },
  { icon: ShieldAlert, titleKey: 'landingpage.benefits.b2Title', descKey: 'landingpage.benefits.b2Desc', color: 'bg-indigo-500/10 text-indigo-500' },
  { icon: Target, titleKey: 'landingpage.benefits.b3Title', descKey: 'landingpage.benefits.b3Desc', color: 'bg-violet-500/10 text-violet-500' },
  { icon: Zap, titleKey: 'landingpage.benefits.b4Title', descKey: 'landingpage.benefits.b4Desc', color: 'bg-amber-500/10 text-amber-500' },
]

// Core features
const coreFeatures = [
  { titleKey: 'landingpage.features.precisioneACWR', descKey: 'landingpage.features.descPrecisioneACWR', icon: Activity, color: 'bg-indigo-500/10 text-indigo-500 dark:text-indigo-400' },
  { titleKey: 'landingpage.features.feedbackLoop', descKey: 'landingpage.features.descFeedbackLoop', icon: Mail, color: 'bg-emerald-500/10 text-emerald-600 dark:text-emerald-400' },
  { titleKey: 'landingpage.features.pianificazione', descKey: 'landingpage.features.descPianificazione', icon: Calendar, color: 'bg-violet-500/10 text-violet-600 dark:text-violet-400' },
  { titleKey: 'landingpage.features.clinicalData', descKey: 'landingpage.features.descClinicalData', icon: ShieldAlert, color: 'bg-orange-500/10 text-orange-600 dark:text-orange-400' },
]
    ...
    <nav class="fixed top-0 inset-x-0 z-50 border-b border-white/[0.06] bg-background/75 backdrop-blur-xl">
      <div class="container mx-auto px-6 py-4 flex items-center justify-between">
        ...
        <div class="hidden md:flex items-center gap-8 text-sm text-muted-foreground">
          <a href="#perche" class="hover:text-foreground transition-colors">{{ t('landingpage.navbar.features') }}</a>
    ...
    <main>
      ...
      <section class="relative pt-36 pb-20 px-6 overflow-hidden">
        ...
        <div class="container mx-auto max-w-4xl text-center">
          ...
          <!-- Headline — benefit first, pain visible -->
          <h1 class="text-5xl sm:text-6xl md:text-7xl font-black tracking-tighter leading-[0.9] mb-6 uppercase">
            <span class="block text-muted-foreground/60 line-through decoration-red-500/60 decoration-4">
              {{ t('landingpage.hero.headline1') }}
            </span>
            <span class="block text-muted-foreground/60 line-through decoration-red-500/60 decoration-4">
              {{ t('landingpage.hero.headline2') }}
            </span>
            <span class="block bg-gradient-to-r from-indigo-400 via-violet-400 to-purple-400 bg-clip-text text-transparent mt-2">
              {{ t('landingpage.hero.headline3') }}
            </span>
          </h1>

          <!-- Subheading — explains the problem solved -->
      ...
      <!-- ─── IL PROBLEMA ──────────────────────────────────────────────── -->
      <section class="py-24 px-6 border-t border-border/40 bg-accent/[0.03]">
        <div class="container mx-auto max-w-5xl">
          <div class="text-center mb-14">
            <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-red-500/10 border border-red-500/20 text-red-400 text-xs font-semibold mb-5">
              <AlertTriangle class="h-3 w-3" />
              {{ t('landingpage.pain.badgeLabel') }}
            </div>
            <h2 class="text-3xl md:text-4xl font-black tracking-tight leading-tight mb-0">
              {{ t('landingpage.pain.title') }}
            </h2>
          </div>
          ...
          <div class="grid sm:grid-cols-2 gap-4 mb-10">
            <div
              ...
              :key="item.titleKey"
              class="flex gap-4 p-6 rounded-2xl border border-border/60 bg-card hover:bg-accent/10 transition-colors"
            >
              <div :class="['size-10 rounded-xl flex items-center justify-center shrink-0 mt-0.5', item.color]">
                <component :is="item.icon" class="size-5" />
              </div>
              <div>
                <h3 class="font-bold text-sm mb-1.5">{{ t(item.titleKey) }}</h3>
                <p class="text-sm text-muted-foreground leading-relaxed">{{ t(item.descKey) }}</p>
              </div>
            </div>
          </div>

          <!-- Outro / bridge to solution -->
          <div class="text-center p-6 rounded-2xl border border-indigo-500/20 bg-indigo-500/[0.04]">
            <p class="text-sm text-muted-foreground max-w-2xl mx-auto leading-relaxed">
              {{ t('landingpage.pain.outro') }}
            </p>
          </div>
        </div>
      </section>
      ...
      <!-- ─── BENEFICI ─────────────────────────────────────────────────── -->
      <section id="perche" class="py-28 px-6 border-t border-border/40">
        <div class="container mx-auto max-w-5xl">
          <div class="text-center mb-14">
            <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-emerald-500 text-xs font-semibold mb-5">
              <CheckCircle2 class="h-3 w-3" />
              {{ t('landingpage.benefits.badgeLabel') }}
            </div>
            <h2 class="text-4xl md:text-5xl font-black tracking-tight uppercase leading-tight mb-4">
              {{ t('landingpage.benefits.title') }}
              <span class="block text-muted-foreground font-light normal-case tracking-normal text-2xl md:text-3xl mt-1">{{ t('landingpage.benefits.titleSub') }}</span>
            </h2>
          ...
          <div class="grid sm:grid-cols-2 gap-5">
            <div
              v-for="b in benefits"
              :key="b.titleKey"
              class="group p-7 rounded-2xl border border-border/60 bg-card hover:bg-accent/20 transition-all duration-300 hover:-translate-y-1 hover:border-border hover:shadow-sm"
            >
              <div :class="['size-11 rounded-xl flex items-center justify-center mb-5', b.color]">
                <component :is="b.icon" class="size-5" />
              </div>
              <h3 class="font-bold mb-2 text-base">{{ t(b.titleKey) }}</h3>
        ...
        <div class="container mx-auto max-w-3xl">
          <div class="text-center mb-14">
            <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-accent border border-border/60 text-muted-foreground text-xs font-medium mb-5">
              <Zap class="h-3 w-3" />
              {{ t('landingpage.howItWorks.badgeLabel') }}
            </div>
            <h2 class="text-4xl font-black tracking-tight uppercase mb-2">
              {{ t('landingpage.howItWorks.title') }}
            </h2>
            <p class="text-muted-foreground text-sm">{{ t('landingpage.howItWorks.titleSub') }}</p>
          </div>

          <div class="space-y-4">
            <div
              v-for="(step, i) in [
                { num: '01', titleKey: 'landingpage.howItWorks.step1Title', descKey: 'landingpage.howItWorks.step1Desc', color: 'text-indigo-400 border-indigo-500/30 bg-indigo-500/10' },
                { num: '02', titleKey: 'landingpage.howItWorks.step2Title', descKey: 'landingpage.howItWorks.step2Desc', color: 'text-violet-400 border-violet-500/30 bg-violet-500/10' },
                { num: '03', titleKey: 'landingpage.howItWorks.step3Title', descKey: 'landingpage.howItWorks.step3Desc', color: 'text-emerald-400 border-emerald-500/30 bg-emerald-500/10' },
              ]"
              :key="i"
              class="flex gap-5 p-6 rounded-2xl border border-border/60 bg-card hover:bg-accent/10 transition-colors"
            >
              <div :class="['size-12 rounded-2xl border flex items-center justify-center shrink-0 font-black text-lg', step.color]">
                {{ step.num }}
              </div>
              <div class="pt-1">
                <h3 class="font-bold mb-1.5">{{ t(step.titleKey) }}</h3>
                <p class="text-sm text-muted-foreground leading-relaxed">{{ t(step.descKey) }}</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <!-- ─── PRODUCT SHOWCASE ─────────────────────────────────────────── -->
      <section id="showcase" class="py-28 px-6 border-t border-border/40">
        <div class="container mx-auto max-w-5xl">
          <div class="text-center mb-14">
            <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-accent border border-border/60 text-muted-foreground text-xs font-medium mb-5">
            ...
            <h2 class="text-4xl md:text-5xl font-black tracking-tight uppercase leading-tight mb-4">
              ...
              <span class="block text-muted-foreground font-light normal-case tracking-normal text-xl md:text-2xl mt-1">{{ t('landingpage.showcase.titleSub') }}</span>
            </h2>
          </div>
            ...
            <Transition name="fade-tab" mode="out-in">
              <div :key="activeTab">
                ...
                <p class="text-center text-sm text-muted-foreground mt-5 max-w-xl mx-auto">
                  {{ t(showcaseTabs[activeTab]?.descKey ?? '') }}
                </p>
              </div>
            </Transition>
          </div>
      ...
      <!-- ─── VS EXCEL ─────────────────────────────────────────────────── -->
      <section id="features" class="py-28 px-6 border-t border-border/40 bg-accent/[0.03]">
        <div class="container mx-auto max-w-4xl">
          <div class="text-center mb-14">
            ...
            <p class="text-muted-foreground max-w-xl mx-auto text-sm">{{ t('landingpage.features.subtitle') }}</p>
          </div>
          ...
          <div class="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
            ...
            >
              ...
              <h3 class="text-sm font-bold mb-2">{{ t(f.titleKey) }}</h3>
          ...
          <div class="grid sm:grid-cols-3 gap-4">
            <div class="p-6 rounded-2xl border border-border/60 bg-card hover:bg-accent/20 transition-all duration-300 hover:-translate-y-1 hover:border-border">
              ...
              <h3 class="text-sm font-bold mb-2">{{ t('landingpage.features.sleepTitle') }}</h3>
            ...
            <div class="p-6 rounded-2xl border border-border/60 bg-card hover:bg-accent/20 transition-all duration-300 hover:-translate-y-1 hover:border-border">
              ...
              <h3 class="text-sm font-bold mb-2">{{ t('landingpage.features.aiTitle') }}</h3>
            ...
            <div class="p-6 rounded-2xl border border-border/60 bg-card hover:bg-accent/20 transition-all duration-300 hover:-translate-y-1 hover:border-border">
              ...
              <h3 class="text-sm font-bold mb-2">{{ t('landingpage.features.injuryTitle') }}</h3>
            ...
            <div class="space-y-7">
              ...
              <div class="grid grid-cols-2 gap-4">
                <div class="space-y-1.5 p-4 rounded-xl border border-border/60 bg-card">
                  <p class="text-xs font-black uppercase tracking-widest text-foreground/80">{{ t('landingpage.scalability.dataIntegrity') }}</p>
                  <p class="text-xs text-muted-foreground font-mono italic">{{ t('landingpage.scalability.dataIntegrityDesc') }}</p>
                </div>
                <div class="space-y-1.5 p-4 rounded-xl border border-border/60 bg-card">
                  <p class="text-xs font-black uppercase tracking-widest text-foreground/80">{{ t('landingpage.scalability.security') }}</p>
                  <p class="text-xs text-muted-foreground font-mono italic">{{ t('landingpage.scalability.securityDesc') }}</p>
                </div>
              </div>
            </div>
      ...
      <!-- ─── PRICING ────────────────────────────────────────────────── -->
      <section id="pricing" class="py-28 px-6 border-t border-border/40 bg-accent/[0.03]">
        <div class="container mx-auto max-w-5xl">
          <div class="text-center mb-14">
            <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-accent border border-border/60 text-muted-foreground text-xs font-medium mb-5">
              <Star class="h-3 w-3" />
            ...
            <p class="text-muted-foreground max-w-xl mx-auto text-sm">{{ t('landingpage.pricing.subtitle') }}</p>
          </div>
          ...
          <div class="grid md:grid-cols-3 gap-6 items-start">
            ...
            <div class="relative rounded-3xl border border-border/60 bg-card p-8 overflow-hidden">
              <div class="absolute inset-0 backdrop-blur-[2px] bg-background/60 rounded-3xl z-10 flex flex-col items-center justify-center gap-3">
                ...
                <span class="text-xs font-semibold text-muted-foreground uppercase tracking-wider">{{ t('landingpage.pricing.lockedBadge') }}</span>
              </div>
              <div class="mb-6">
              ...
              <Button disabled class="w-full h-11 rounded-xl" variant="outline">{{ t('landingpage.pricing.ctaLocked') }}</Button>
            </div>
      ...
      <!-- ─── ROADMAP ───────────────────────────────────────────────────── -->
      <section id="roadmap" class="py-28 px-6 border-t border-border/40">
        <div class="container mx-auto max-w-3xl">
          <div class="text-center mb-14">
      ...
      <!-- ─── WAITLIST / CTA FINALE ────────────────────────────────────── -->
      <section id="waitlist" class="py-28 px-6 border-t border-border/40 bg-accent/[0.04]">
        <div class="container mx-auto max-w-xl text-center">
          <!-- Badge urgency -->
          <div class="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-indigo-500/10 border border-indigo-500/20 text-indigo-400 text-xs font-semibold mb-6">
            <span class="size-1.5 rounded-full bg-indigo-400 animate-pulse" />
          ...
          <p class="text-muted-foreground text-base mb-10 max-w-md mx-auto leading-relaxed">
            {{ t('landingpage.waitlist.subtitle') }}
          </p>

          <!-- Success state -->
          <div v-if="isSubmitted" class="p-8 rounded-3xl border border-emerald-500/30 bg-emerald-500/5">
            <CheckCircle2 class="h-10 w-10 text-emerald-500 mx-auto mb-4" />
            <h3 class="text-lg font-bold mb-2">{{ t('landingpage.waitlist.successTitle') }}</h3>
            <p class="text-sm text-muted-foreground mb-5">{{ t('landingpage.waitlist.successMessage') }}</p>
            <button
          ...
          <!-- Form -->
          <form v-else class="flex flex-col sm:flex-row gap-3 justify-center" @submit.prevent="handleFormSubmit">
            <div class="flex-1">
              <Label for="waitlist-email" class="sr-only">{{ t('landingpage.waitlist.emailLabel') }}</Label>
              <Input
                id="waitlist-email"
                v-model="email"
                type="email"
                :placeholder="t('landingpage.waitlist.placeholder')"
                class="h-12 rounded-xl bg-card border-border/60 px-4 text-sm w-full"
                required
              />
            </div>
            <Button
              ...
              class="h-12 px-8 rounded-xl bg-gradient-to-r from-indigo-500 to-violet-600 hover:from-indigo-400 hover:to-violet-500 text-white font-bold border-0 shadow-lg shadow-indigo-500/25 transition-all hover:-translate-y-0.5 shrink-0"
            >
              <Loader2 v-if="isSubmitting" class="h-4 w-4 animate-spin mr-2" />
              {{ isSubmitting ? t('landingpage.waitlist.submitting') : t('landingpage.waitlist.submitButton') }}
            </Button>
          </form>

          <p class="text-xs text-muted-foreground/50 mt-5 flex items-center justify-center gap-2">
            <CheckCircle2 class="h-3.5 w-3.5 text-emerald-500" />
            {{ t('landingpage.waitlist.noSpam') }}
          </p>
        </div>
      </section>
