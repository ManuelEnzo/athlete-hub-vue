...
import { Activity, Loader2, User } from 'lucide-vue-next'
        ...
        <div class="flex items-center gap-3">
          ...
          <div>
            ...
            <p class="text-xs text-muted-foreground mt-0.5">
              {{ t('athlete.visualizerDescription') || 'Analizza le performance e i dati di salute degli atleti' }}
            </p>
          </div>
        </div>
    ...
    <div class="px-6 py-6">
      ...
      <div v-else-if="!loadingAthletes" class="py-24 flex flex-col items-center gap-3 text-center">
        ...
        <div>
          ...
          <p class="text-xs text-muted-foreground/60 mt-1">{{ t('athlete.selectPrompt') || 'Seleziona un atleta dal menu in alto per visualizzare i dati' }}</p>
        </div>
      </div>
    </div>
  </div>
