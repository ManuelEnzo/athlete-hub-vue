...
interface AthleteHealth {
    ...
    readinessScore: number; avgWellness: number;
...
const sortedTeamHealth = computed(() => {
    return [...mockTeamHealth].sort((a, b) => {
        let valA = sortBy.value === 'acwr' ? a.acwr : a.readinessScore
        let valB = sortBy.value === 'acwr' ? b.acwr : b.readinessScore
    ...
    <div class="w-full flex flex-col gap-6">
        ...
        <div class="grid grid-cols-1 gap-6 xl:grid-cols-3">
                ...
                <CardContent class="p-0">
                    <table class="w-full">
                        ...
                        <tbody class="divide-y">
                            <tr v-for="athlete in sortedTeamHealth" :key="athlete.id"
                                class="transition-colors hover:bg-muted/50">
                                <td class="py-4 px-4">
                                    <div class="font-semibold">{{ athlete.name }}</div>
                                    <div class="text-xs text-muted-foreground">{{ athlete.position }}</div>
                                </td>
            ...
            <div class="flex flex-col gap-6">
                ...
                <Card>
                    <CardHeader>
                        <CardTitle class="text-sm">Distribuzione Rischio</CardTitle>
                    </CardHeader>
